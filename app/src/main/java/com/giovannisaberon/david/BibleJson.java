...
//        Log.i("json bible: ", json);
    ...
    public JSONObject getJsonBible(String filename) throws JSONException {
        String bible = null;
        try {
            bible = this.loadJSONFromAsset(filename);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public String getVerse(JSONObject jsonBible, String book, String chapter, int verse) throws JSONException {
        JSONObject jsonbook = jsonBible.getJSONObject(book);
        JSONArray jsonchapter = jsonbook.getJSONArray(chapter);
        JSONObject v = jsonchapter.getJSONObject(verse-1);
